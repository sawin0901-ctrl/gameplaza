...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    queue: { db: queuedItems, waiting: queueStats.waiting, active: queueStats.active, failed: queueStats.failed },
    today: { imported: todayLog?.imported ?? 0, updated: todayLog?.updated ?? 0, errors: todayLog?.errors ?? 0 },
  })
}