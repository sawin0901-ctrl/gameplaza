    ...
    <div ref={wrapperRef} className="relative" style={{ minHeight: "300px" }}>
      ...
      {timedOut && (
        <div className="mt-4 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 text-center shadow-sm">
          ...
          <p className="text-sm font-medium text-[var(--text)]">Кнопка покупки загружается</p>
          <p className="mt-1 text-xs text-[var(--text-muted)]">Если она так и не появилась — попробуйте обновить страницу</p>
          <button
            onClick={() => window.location.reload()}
            className="mt-3 inline-flex items-center gap-2 rounded-xl bg-[var(--brand)] px-5 py-2 text-sm font-semibold text-white transition-opacity hover:opacity-80"
          >
